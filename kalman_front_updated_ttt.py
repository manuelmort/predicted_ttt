...
================================================
Light mode — scene only with in-axes HUD
Control law:  u = K * (τ̂^left_{k|k} − τ̂^right_{k|k})
  Uses KF posterior estimate only — no lookahead.
"""
...
while obs_count < N_OBS:
    if frame_idx % TAU_INTERVAL == 0:
        ...
        # KF LEFT — update to get τ̂^left_{k|k}
        if z_kf_L is None:
        ...
        # KF RIGHT — update to get τ̂^right_{k|k}
        if z_kf_R is None:
...
ctrl_t  = ax.text(0.01, 0.65, "", color="#444444", **hud_kw)
...
def animate(frame):
    ...
    ctrl_t.set_text(
        f"u = K·(τ̂^L_{{k|k}} − τ̂^R_{{k|k}})\n"
        f"  τ̂^L = {tau_lp:+.4f}   τ̂^R = {tau_rp:+.4f}\n"
        f"  Δτ  = {tau_lp - tau_rp:+.4f}\n"
        f"  u_k = {u_k:+.4f} rad   θ = {np.degrees(th):+.2f}°")
